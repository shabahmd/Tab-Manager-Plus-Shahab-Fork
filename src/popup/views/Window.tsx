...
import {normalizeContainerId, ALL_CONTAINERS} from "@helpers/containers";
...
export class Window extends React.Component<IWindow, IWindowState> {
	...
	isHiddenByContainer(tab : browser.Tabs.Tab) : boolean {
		const filter = this.props.containerFilter;
		if (!filter || filter === ALL_CONTAINERS) return false;
		return normalizeContainerId(tab) !== filter;
	}
		...
		let tabs = [...this.props.tabs].sort((a, b) => {
			...
			return (
				<Tab
					...
					selected={isSelected}
					hidden={isHidden}
					containerColors={_this.props.containerColors}
			...
			if (!!this.props.tabactions) {
				tabs.push(
					...
					<div key={"windowactions_" + this.props.window.id} className="window-actions">
						...
						/>
						<div
							className={"icon tabaction hibernate " + (this.props.layout.indexOf("blocks") > -1 ? "" : "windowaction")}
							title="Hibernate window&#10;Discard inactive tabs to free RAM"
							onClick={() => browser.runtime.sendMessage<ICommand>({command: S.hibernate_window, window_id: this.props.window.id})}
							onMouseEnter={this.props.hoverIcon}
						>
							💤
						</div>
	...
	addTab(e) {
		...
		if (navigator.userAgent.search("Firefox") > -1) {
			browser.runtime.sendMessage<ICommand>({command: S.create_tab, window_id: this.props.window.id});
		} else {
	...
	async save(e) {
		...
		this.props.parentUpdate();
		debugLog("Value is set to " + value);
