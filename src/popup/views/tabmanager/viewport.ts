...
export function scrollToElement(host: ViewportHost, what : string, id : number): void {
	const els = document.getElementById(what + "-" + id);
	if (els) {
...
export function isElementVisible(elem : HTMLElement): boolean {
	...
	const _opacity : number = parseFloat(style.opacity);
	...
	let pointContainer : ParentNode = document.elementFromPoint(elemCenter.x, elemCenter.y);
	do {
...
export function scrollActiveTabIntoView(animations: boolean): void {
	setTimeout(async function() {
		const scrollArea = document.getElementsByClassName("window-container")[0];
		const activeWindow = document.getElementsByClassName("activeWindow");
		if (!!activeWindow && activeWindow.length > 0) {
			const activeTab = activeWindow[0].getElementsByClassName("highlighted");
